...
void initBuilding(BuildingType *building){
	...
	building->allHunterCount = 0;
	initRoomList(&building->rooms);
	initEvidenceList(&building->allEvidence);
}

/*
 Function:   freeBuilding
  Purpose:   free all the data and the nodes created in the building (evidence data, room data, building nodes, hunter nodes, room nodes)
   in/out:   pointer to the building
   return:   nothing, is void
*/
void freeBuilding(BuildingType* building){
	...
	while(curr!=NULL){
		freeEvidenceNodes(&curr->data->roomEvidence);
		curr = curr->next;
	}
	for(int i = 0; i < building->allHunterCount; i++){
		freeEvidenceNodes(&building->allHunters[i]->myEvidence);
	}
	
	freeEvidenceData(&building->allEvidence);
	freeEvidenceNodes(&building->allEvidence);
	
	
	
	
	RoomNodeType *curr2 = building->rooms.head;
	
	
	while(curr2!=NULL){
		freeRoomNodes(&curr2->data->connectedRooms);
		curr2 = curr2->next;
	}
	freeRoomData(&building->rooms);
	freeRoomNodes(&building->rooms);
}


/*
 Function:   populateRooms
  Purpose:   populate the rooms inside the building
   in/out:   pointer to the building
